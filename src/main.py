...
def print_age(age_years: int, age_months: int, age_days: int) -> None:
    if not age_years:
        age_years = ""
    elif age_years == 1:
        age_years = "1 rok "
...
def age_calc() -> None:
    try:
        ...
        print_age(age_years, age_months, age_days)

    except ValueError:
        print("\nPodano nieprawidłową datę!")


def weekday_check() -> None:
    date_today = datetime.now(tz=timezone.utc)
    days = {
        0: "Poniedziałek",
        1: "Wtorek",
